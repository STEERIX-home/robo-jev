...
기준: `main` @ `df5e00a`(PR #1 merge) 이후 문서 커밋 포함, 이 파일이 있는 커밋. 검증 상태: `uv run pytest -q` → **계약 v0.3 브랜치(task-v03)에서 950 passed, 0 xfailed, 0 skipped (197 s)**(E1 seed 29의 xfail은 지시 조합 예약이 고쳐 지웠다; tokenizer가 있어야 skip 0 — 보고서 `.superpowers/sdd/task-v03-contract-report.md`). Spark 인수는 2026-09-19 완료(§3-1~4).
...
1. **확정(2026-09-19, Task v0.3)** — 계약 v0.3: 서식 축약 + 변화분 틱 + 후보 공간 축소(결합 키에서 `profile` 제거, 영역 쪽 push, 지시 조합 예약, K≤12) + 비키프레임 비용 허용 집합(τ=0.15)·hold∉A. 실측: 10물체·K=12에서 틱당 p50 392 / p95 662 / 첫 틱 943, 100틱 43K(08 §3.4); E1 sweep 퇴화 틱 49.7% → 0%, 완료 20/26 → 23/26. 버전 h0.4·ts0.4·e0.3·rj0.4, batch-0 재생성(40편, E0·E1 40/40 완료).
2. **확정(2026-09-19, Task v0.3)** — holdout 봉인(04 §5 표): 분야마다 템플릿 변형 하나·개념 하나, 로봇은 zoneF 목표 계열 + 지시 변형 3번 + E1 계열 하나; OOD는 `ood_dev`/`ood_test`로 반분, QA가 누출 0을 확인한다. 비로봇 개념 어휘의 선택(spatial goal-zone zoneC, dom reveal, workflow resource_offline, rules escort-policy)은 보고서(`.superpowers/sdd/task-v03-contract-report.md`)에서 확인.
...
2. tokenizer를 **같은 revision·해시로** 받는다(아래 §4의 값; `uv run python scripts/fetch_tokenizer.py --from-manifest artifacts/tokenizers/manifest.json` — manifest 파일을 먼저 복사; 또는 `--revision … --expect-sha256 …`; 다른 후보로의 fallback은 `--allow-fallback`을 줄 때만). **tokenizer 확보 뒤에** `uv run pytest -q`(950 passed, 0 xfailed 기대; skip이 남으면 tokenizer가 없는 것).
...
5. **완료(2026-09-19, Task v0.3)** 결정 1~2 반영 → 하네스·직렬화 v0.3 구현 → batch-0 재생성 → B2 재측정(`artifacts/reports/tokens-b2.json`, 옛 값은 `tokens-b2-v0.json`) → 2b 재실행(`uv run python scripts/measure_candidates.py --config configs/model/candidates.yaml --path native --candidates Qwen/Qwen3.5-2B,Qwen/Qwen3.5-4B --profiles lower,upper,instruction_change --ticks 70 --report artifacts/reports/backbone-screen-v03.json`; `v03_target` 대역은 은퇴). **다음**: G0b(2B·4B의 `stream` 경로: 정적 윈도우 KV, CUDA graph/`torch.compile`, 마스크 없는 attention 커널 — sdpa+마스크가 윈도우 틱의 ≈40%; readout-only 적응·무학습 품질) → `--limit 1000` rollout sweep → 128k → 클라우드 학습.
