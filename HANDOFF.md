...
checkpoint는 읽지도 쓰지도 않아 무사하다. 233 step 대신 466을 돌았으니 **≈4.0 GPU-h(실측 3.97 h: 8.11 h 대 233 step run 평균 4.14 h)가 더 갔다**. 비교는
...
**Task R4(2026-09-24)가 학습된 모델을 루프에 꽂았다 — 첫 폐루프 값이고, 오프라인 칸이 못 본 것이 나왔다.** 학습·데이터·하네스·컨트롤러·직렬화는 그대로(h0.9·c0.6·ts0.6). 더한 것: `model/incremental.py`(증분 ts0.6 직렬화 — 전체 직렬화와 토큰 단위로 같다, 검사), `harness/model_policy.py`(`ModelPolicy.act` — 정적 prefix KV + 30틱 윈도우 + 결정 분기 배치 readout, expert와 같은 답 꼴; `load_serving_judge`는 계약 digest를 대조하고 compile은 그 뒤), `closed_loop.py` + `scripts/closed_loop.py {verify,seeds,run,report,transitions}` + `configs/eval/r4-*.yaml`. **A3**: 녹화 dev 3편 389틱 3,890답에서 루프 정책 = 오프라인 재생 **3,890/3,890, 확률 차 0**(`fused`; compile을 더한 `all`은 |Δp| ≤ 0.019, argmax 동일), R3a 저장 예측과 389/389. **A4**: 루프 안 모델 p50/p95/p99 **43.6/56.8/82.7 ms**, 100 ms 초과 0/7,171, 관측→명령 p50 48 / p95 63 ms. **B**(dev 100 / ood_dev 26, 새 seed 900100~): expert 0.970 [0.930, 1.000] / 1.000 · 규칙 0.800 [0.720, 0.870] / 0.692 [0.500, 0.846] · 기계적 0 / 0 · **모델 seed 18 0 / 0** · 466 0.020 [0, 0.050] / 0(그리퍼를 닫지 않고 done 게이트가 든 2편). 짝지은 차 규칙 − 모델 **+0.800 [+0.720, +0.870]**. **까닭은 부가 판단 하나다**: 모델은 참조가 결합 후보를 허용하는 7,041틱에서 채택 행동이 91.8 % 같고 목표 변경 즉시 반응 95.9 %(규칙 79.9 %)인데 **파지점에서 `closed`를 한 번도 내지 못해**(참조 전환 125건 중 실행 0) 팔이 열린 채 머물다 정체 감시(`stall_exhausted`)로 끝난다 — 실패 100편 = 부가 판단 90(그리퍼 81) · 주 결정 8 · 기하 2. 같은 checkpoint의 오프라인 `q_gripper` **0.996**: 닫힌 뒤의 실행 상태(`exec g=closed`)를 베끼면 맞는 틱이 거의 전부이고 "지금 닫아라" 틱은 파지마다 몇 틱뿐(라벨의 전환 허용 ±1틱이 그마저 반쯤 지운다). 규칙 판정기의 실패 28편도 같은 종류(`rj0.5` 파지점 문턱). **규칙 기준군은 지시 변경 없는 층(E0)만 포화**(변경 층 expert 대비 −0.213 [−0.312, −0.125] / −0.381 [−0.619, −0.190]) → 과제 재설계는 G1에 올리지 않는다. `q_stop`: seed 18은 8건 중 6건, 오경보 1.1 %가 정지 틱 82건(반사 4건); 안전 위반 1.60 %(규칙 0.0 %). **첫 run report** `docs/reports/run-report-1.md`(가중치·데이터·재개·품질·폐루프·비용 한 문서) — **권고: 클라우드로 seed·데이터를 키우지 않는다; 먼저 부가 판단(그리퍼 전환)의 평가 칸·라벨을 고친다.** 비용 GPU 0.63 h(A3 2 분 + run 12 + 11 분 + 같은 seed 오프라인 13 분), CPU 15 분. 폐루프 기록 `artifacts/datasets/r4-closed-loop/{s18,466}/`(126편 × 2, `labels` = expert 참조)이 **첫 DAgger 데이터 후보**다. 보고서 `.superpowers/sdd/task-r4-report.md`.
