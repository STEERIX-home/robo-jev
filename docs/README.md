...
| 2026-09-21 Task P1 (첫 학습 파일럿) | Task 5의 GPU 인수 검사를 실제 2B에서 끝냄(T0 동결 320 tensor 0 변경, LoRA·T1 gradient·parameter 이동, 저장 후 **프로세스 재시작** 재개가 비트 동일 → G0 통과, 200 step profile 9.94 s/step·10.08 GiB) + 06 Task 5의 200 step YAML을 실제로 돌린 설정으로 정정; 프로파일 상한 Q≤16·K≤32을 `validate_record`가 강제(D1 최댓값 스트림 K≤12·비로봇 Q=16); **고정 평가 집합**(`configs/eval/pilot.yaml`, 해시로 run을 묶는다)과 그 위에서의 D1 like-for-like 7 run — 판정 칸 `robot/ood_dev` q_main에서 4B T0 0.871(상태 섞기 0.799) 대 2B T0 0.454(0.289), 2B는 LoRA 0.570·T1 0.712까지 가도 못 미친다 → **4B를 품질 문제에서 물리지 않는다**(backbone 결정은 지연이 정한 것이라 그대로, 5 Hz는 M-d 전제 아래 재개); 06:157(무학습 라벨 점수) 체크; BF16 master weight 때문에 T1이 40 step·1e-5에서 표본의 4.83 %만 움직인다는 실측(진짜 T1에는 fp32 master weight 필요); 03 §7의 1·5·7 상태 | [06 Task 5·2b](06-execution-roadmap.md), [03 §7](03-model-and-training-design.md), `artifacts/reports/p1-*.json`, `.superpowers/sdd/task-p1-report.md` |
