...
로봇 스트림 학습은 에피소드를 10초 구간(약 100틱; 실측 서식으로 176K~371K token, 계약 v0.3 목표로 ≈50K — 55K 가정은 v0.3 이후 값)으로 나눠 같은 optimizer step 안에서 상태를 전달하는 truncated BPTT로 수행하므로, 구간당 활성 메모리와 full-attention 윈도우(정적 prefix + 30틱)의 KV(27B 기준 약 1.0GiB, 분기·임시 메모리 제외)를 후보 backbone의 메모리 기준에 넣는다. 이 조건은 27B보다 9B 후보에 유리하며 Task 2b의 선정 기준이다. 지연 게이트(G0b의 10Hz·100ms 초과율)는 배포·실행 장비가 DGX Spark/Jetson급 엣지이므로 **그 장비에서** 잰다(클라우드는 학습 처리량만); 엣지에서는 27B가 본선이 아니고 2~4B 또는 9B(FP8 별도 조건)를 속도·무학습 품질로 정한다.
