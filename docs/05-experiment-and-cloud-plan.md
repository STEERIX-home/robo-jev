...
| full (checkpointing) | 10 s | 45,475 | **47.3 GiB** / 48 s | **OOM**(73 GiB 울타리; 기울기 ≈1.4 MB/토큰 → ≈90~105 GiB 추정) |
...
**확정 backbone 기준 재산정(2026-09-20, G0b; 2B 주·4B 5 Hz 대비).** 위 표의 27B 기준 노드·단가는 더 이상 본 실험의 것이 아니다. GB10 실측 처리량(05 §4 표, 45.5K 토큰 10초 구간): readout-only 2B ≈5.9K / 4B ≈2.8K 토큰/s, 층 단위 checkpointing full 2B ≈950 / 4B(5초 구간) ≈475 토큰/s; LoRA(r=16, backbone 고정)는 activation을 backward까지 들고 있으므로 full과 같은 자릿수의 시간이다 — G0b 실측 step(로봇 5초 구간 + 비로봇 8K 단위, accumulation 2): 2B 59 s/step(27 GiB); 4B 120 s/step(65 GiB). 계약 v0.3의 실제 토큰(batch-0 train 26편 = 2,228틱 = 936K 토큰 → 에피소드당 ≈36K; pilot 1,265상태 = 1.54M 토큰)으로 D1(로봇 400편 ≈14M + 비로봇 2K상태 ≈2.4M, 단일 요청은 P0 경로 복제로 ≈8× 처리) 1 epoch는 2B full/LoRA에서 GB10 ≈ **10~12시간**, readout-only ≈ 1.5시간 — 즉 **파일럿 규모의 T0와 2B/4B LoRA는 Spark 한 대에서 돈다**(HANDOFF 결정 4 "첫 실가중치 학습은 클라우드"는 파일럿에 한해 다시 본다: 클라우드는 D2 규모·seed 반복·4B full에 쓴다). 80 GB GPU 하나는 2B 10초 구간 full(47 GiB)·4B 5초 구간(58 GiB)을 받으며, 8×H100 노드는 2B에 필요하지 않다 — G1 이후의 노드 구성·단가는 GB10 대비 H100의 실측 배수가 나온 뒤(첫 클라우드 run의 tokens/s) 다시 계산하고, 그전에는 아래 27B/9B 표를 상한 참고로만 둔다.
