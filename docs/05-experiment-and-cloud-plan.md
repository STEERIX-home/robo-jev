...
위 용량표는 첫 후보 27B dense 기준의 상한 산정이다. 후보가 바뀌면 다시 계산한다. 작은 dense 후보(예: 약 9B)는 학습 상태 계획값이 약 144GB로 줄어 더 작은 노드로도 가능하다. MoE 후보는 추론 연산이 active 파라미터에 비례하지만 **학습 메모리는 총 파라미터에 비례**한다. 예를 들어 총 35B의 MoE는 `16P` 기준 약 560GB로 8×80GB에서 activation 여유가 거의 없으므로 8×H200 또는 8×B200급 노드가 필요할 수 있다. 후보별 학습 노드와 비용은 선정 시점에 같은 방법으로 산정한다. peak allocated/reserved, 최대 rank의 메모리, gradient accumulation의 영향을 측정한다. Spark native 추론 실측에서는 peak allocated 13.7 / 28.3 / 37.9 GiB(2B/4B/9B)에 reserved가 54.8 / 57.0 / 83.7 GiB로 2~4×였다 — 틱마다 자라는 KV의 `torch.cat`이 새 segment를 만드는 allocator 현상이라(`num_alloc_retries`·`num_ooms` 0) 2단계는 정적 윈도우 KV를 미리 할당한다. 10초 학습 구간 activation의 하한 추정(토큰 × hidden × 층 × 2 B)은 50K 토큰에서 4.6 / 7.6 / 12.2 GiB, 실측 서식 184,844토큰에서 16.9 / 28.2 / 45.1 GiB였다(추정). **G0b 실측(2026-09-20, GB10, `scripts/adapt_readout.py --mode chunk-memory`, `artifacts/reports/chunk-memory-{2b,4b}.json`; batch-0 최장 에피소드의 첫 구간, 실제 `stream_l1a` TBPTT 경로 forward+backward, BF16 가중치·fp32 readout, 통합 메모리 울타리 0.6 = 73 GiB 안에서):**

| 모드 | 구간 | 토큰 | 2B peak allocated / fwd+bwd | 4B peak allocated / fwd+bwd |
| --- | --- | --- | --- | --- |
| readout-only (T0; backbone 고정, 정적 윈도우 KV) | 10 s (100틱) | 45,475 | **6.2 GiB** / 7.7 s | **14.4 GiB** / 16.5 s |
| full (backbone + readout gradient, **층 단위 activation checkpointing**) | 5 s | 23,037 | 25.3 GiB / 22 s | 58.2 GiB / 49 s |
| full (checkpointing) | 10 s | 45,475 | **47.3 GiB** / 48 s | **OOM**(73 GiB 울타리; 5초까지의 기울기 ≈2.3 MB/토큰(2초→5초) — dynamic KV 모드가 틱마다 윈도우 K/V를 이어 붙인 tensor를 checkpoint 입력으로 들고 있어 처음 30틱은 제곱으로 자란다 → 10초 ≈90~105 GiB 추정) |
| full, checkpointing 없음 | 1 s / 2 s | 4,710 / 8,978 | 24.5 / 41.3 GiB (≈4.1 MB/토큰 → 45K에서 ≈190 GB) | 53.1 GiB / OOM (≈7 MB/토큰) |

읽는 법: checkpointing 없는 full 학습은 10초 구간에서 2B도 190 GB라 어느 단일 GPU에도 들지 않으며(통합 메모리 121 GB의 GB10에서는 커널 OOM으로 세션이 죽었다 — 그래서 `robo_jev.gpu`의 allocator 상한이 모든 GPU 진입점에 걸린다), 층 단위 checkpointing(재계산 비용: fwd+bwd가 forward의 ≈4.7×)이 기본이다. LoRA는 backbone gradient가 없어도 전 층 activation을 backward까지 들고 있어야 하므로 full과 같은 자릿수(가중치 gradient 3.5 / 7.8 GiB만 빠진다). 따라서 **2B는 10초 구간 full/LoRA가 GB10과 80 GB GPU 하나에 여유 있게 들고, 4B는 5초 구간(또는 반으로 나눈 gradient accumulation)이어야 80 GB에 든다**; `16P` 계획값(2B 30 GB, 4B 67 GB)은 이 실측 peak에 밀려난다. 4B full 10초 구간의 나머지(dynamic KV 모드가 틱마다 윈도우 K/V를 이어 붙인 tensor를 checkpoint 입력으로 들고 있는 ≈0.3 MB/토큰)는 조각을 그대로 넘기면 줄일 수 있다(G0b 보고서 미결).
...
**확정 backbone 기준 재산정(2026-09-20, G0b; 2B 주·4B 5 Hz 대비).** 위 표의 27B 기준 노드·단가는 더 이상 본 실험의 것이 아니다. GB10 실측 처리량(05 §4 표, 45.5K 토큰 10초 구간): readout-only 2B ≈5.9K / 4B ≈2.8K 토큰/s, 층 단위 checkpointing full 2B ≈950 / 4B(5초 구간) ≈475 토큰/s; LoRA(r=16, backbone 고정)는 activation을 backward까지 들고 있으므로 full과 같은 자릿수의 시간이다 — G0b 실측 step(로봇 5초 구간 + 비로봇 8K 단위, accumulation 2): 2B 59 s/step(27 GiB); 4B 120 s/step(65 GiB). 계약 v0.3의 실제 토큰(batch-0 train 26편 = 2,228틱 = 936K 토큰 → 에피소드당 ≈36K; pilot 1,265상태 = 1.54M 토큰)으로 D1(로봇 400편 ≈14M + 비로봇 2K상태 ≈2.4M, 단일 요청은 P0 경로 복제로 ≈8× 처리) 1 epoch는 2B full/LoRA에서 GB10 ≈ **10~12시간**(10초 구간 chunk-memory 실측의 ≈950 토큰/s에서 유도한 계획값이며 학습 run의 실측이 아니다; LoRA 2B 실측 step은 59 s = 5초 구간 + 8K 비로봇 단위), readout-only ≈ 1.5시간 — 즉 **파일럿 규모의 T0와 2B/4B LoRA는 Spark 한 대에서 돈다**(HANDOFF 결정 4 "첫 실가중치 학습은 클라우드"는 파일럿에 한해 다시 본다: 클라우드는 D2 규모·seed 반복·4B full에 쓴다). 80 GB GPU 하나는 2B 10초 구간 full(47 GiB)·4B 5초 구간(58 GiB)을 받으며, 8×H100 노드는 2B에 필요하지 않다 — G1 이후의 노드 구성·단가는 GB10 대비 H100의 실측 배수가 나온 뒤(첫 클라우드 run의 tokens/s) 다시 계산하고, 그전에는 아래 27B/9B 표를 상한 참고로만 둔다.
...
**`prepare`는 아무것도 구속하지 않는 명세를 만들지 않는다.** 상한을 하나도 주지 않거나(마감이 `null`), 나눌 값이 0이라 그 상한이 버려지면(GPU 0에 `--max-gpu-hours`, 단가 0에 `--max-usd`) `prepare`가 **거절한다**(종료 코드 1, 명세도 묶음도 남지 않는다). 특히 `--max-usd`와 `--hourly-usd 0`을 함께 주는 것은 돈 상한을 적어 놓고 버리는 짝이므로 언제나 거절이다 — 콘솔의 단가를 적거나 시간 상한으로 바꾼다. 상한 없이 돌릴 작정이면 `--no-cap`으로 **이름을 불러야** 하고, 그 사실이 명세의 `notes`와 `prepared` 줄에 남는다.

**상한은 멈추는 방아쇠이지 천장이 아니다.** 러너는 step·구간 경계에서만 멈출 수 있으므로 마감을 **한 경계만큼** 넘길 수 있다(R3b의 USD 드릴: `--max-usd 0.02`에 `estimated_usd 0.025234`로 끝났다 — +26 %, 다만 2.9 s짜리 상한에 1.4 s짜리 step이었다). 클라우드 규모에서는 R2의 한 step이 66.76 s ≈ $0.046이라 $12.5 상한에 견주면 무시할 만하지만, `max_usd`를 센트 단위로 재 놓지는 않는다.
...
**카드 크기, 이 run의 것.** 2B의 fp32 master T1은 R2 실측 peak allocated **56.05 GiB** / reserved 57.68 GiB다 — 80 GB 카드(= 74.5 GiB)에 들고 여유는 **reserved 기준**(74.5 − 57.68) 16.8 GiB = **22.6 %**로 — allocated로 재면 18.45 GiB = 24.8 %이지만 할당기가 실제로 붙잡고 있는 것은 reserved이므로 보수적인 쪽을 쓴다 — §4의 "실사용 장치 메모리의 최소 10 %가 남는다" 기준을 통과한다(다만 GB10 통합 메모리에서 잰 값이므로 첫 run의 200 step에서 다시 확인한다). **4B의 fp32 master T1은 이 카드에 들지 않는다**: optimizer step의 바닥만 **94.0 GiB**(Task P2 산술, 03 §5)로 activation 이전에 이미 74.5 GiB를 넘는다. 필요한 것은 **H200 141 GB(131 GiB) 또는 B200 180 GB 한 장**, 아니면 optimizer/parameter sharding(FSDP)인데 이 학습기는 `world_size: 1`이라 그것부터 만들어야 한다. 저장은 checkpoint 하나가 26.35 GB이고 `checkpoint_every 50` + `keep_steps [40]`이면 디스크에 두 개(52.7 GB)가 남는다 — §7의 영속 볼륨에 두고 **회수 시간과 그 재시도를 비용에 더한다**: 26 GB 전송은 몇 분이 아니라 몇십 분이고, 끊기면 다시 붙는 동안에도 인스턴스 요금은 나간다. `fetch`/`push`의 rsync는 `--partial-dir`로 **이어 받으므로** 재시도가 처음부터 다시 받지는 않는다.
