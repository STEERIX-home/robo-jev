...
* ``prefix`` key — 언제나 본다. 처음의 정적 prefix(지시·질문 세트·정적 후보)와 도중에 덧붙인
  지시가 여기 속하며 윈도우 밖으로 내보내지 않는다.
* 그 밖의 key(틱 몸통: 상태·실행 이력·동적 후보) — ``tick[q] - tick[k] < window_ticks``일 때
  본다. 기본 윈도우는 최근 30틱(자기 틱 포함, docs/08 §3.1 "정적 prefix + 최근 3초")이다.
