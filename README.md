...
| Model | contracts, masks, state forking, readout and losses on a random-weight fixture; the real stream path on Qwen3.5-2B/4B (static prefix KV + 30-tick window buffer, DeltaNet state carried through fla, mask-free varlen flash attention, batched decision branches, pointer readout) with a measured BF16 tolerance, branch isolation and next-tick invariants; **the D1 pilot (2026-09-21, Task P1): seven runs on one fixed evaluation set (`configs/eval/pilot.yaml`, `eval_set.sha256 79d09793eab5`) at the same data, steps and seed. On the decision cell — `robot/ood_dev q_main`, the one the Task 2c tiny scorer does **not** call pattern-solvable, with its state-shuffle control in brackets and a rule judge at 0.821 — 4B T0 (200 steps) **0.871** (0.799) against 2B T0 **0.454** (0.289); the 2B reaches 0.570 (0.402) with LoRA and 0.712 (0.601) with a full text-backbone T1, both at 40 steps, so more adaptation does not close the gap. Zero-shot label scoring is also better on the 4B on every split (robot dev 0.776 vs 0.484). No adapted run beats its own zero-shot on the contrast `choice` type, and no run changes its `q_main` answer on any of the 6 instruction contrast pairs** | a backbone trained at scale; fp32 master weights (BF16 master weights lose an AdamW step smaller than half the local quantum — at `backbone_lr 1e-5` only 4.8 % of the sampled backbone moved in 40 T1 steps); calibration |
| Training | step, TBPTT, sampler, checkpoint, resume on the fixture; on the real 2B/4B: readout-only, peft-LoRA **and full text-backbone (T1)** runs on Spark with candidate-order permutation augmentation, per-layer activation checkpointing and a unified-memory guard. **Task 5's GPU acceptance is done (2026-09-21): T0 leaves all 320 backbone tensors bit-identical and saves only the readout; LoRA and T1 put non-zero gradients on every trainable tensor and move them; 20 steps continuous vs 10 + process restart + 10 is bit-identical (loss, sampler position, drawn records, optimizer step, parameters) → G0 passes. Measured cost at D1 scale: 2B T0 200 steps 9.9 s/step / 10.1 GiB / 4,664 tok/s, 4B T0 22.1 s/step / 20.8 GiB, 2B LoRA 63.0 s/step / 26.8 GiB, 2B T1 57.1 s/step / 60.0 GiB, 4B LoRA 131.9 s/step / 64.8 GiB** | cloud-scale training; fp32 master weights for a faithful T1; calibration |
...
tests/       1061 tests; fixtures under tests/fixtures
...
3. ~~First trained model: readout-only and LoRA on the pilot fit on the Spark itself~~ — done (2026-09-21, Task P1: 2B/4B × T0, LoRA, zero-shot plus a 2B full text-backbone T1, ≈9.7 GPU-hours on the Spark; see Status). The cloud is for D2 scale, seeds, 4B full training and a T1 with fp32 master weights. Calibration is still open (the pilot reports uncalibrated ECE only).
